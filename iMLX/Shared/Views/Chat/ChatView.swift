...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .modifier(
            ChatCameraPickerPresentationModifier(
        ...
        .modifier(ChatFileDropModifier(onDrop: handleDroppedFiles(_:)))
        .sheet(item: $utilitySheet) { sheet in
            utilitySheetView(sheet)
        }
        .modifier(
            ChatLiveVoicePresentationModifier(
        ...
        .modifier(
            ChatPlatformLifecycleModifier(
                conversationId: conversationId,
                appState: appState,
                openModels: openModels,
                focusComposer: { isInputFocused = true }
            )
        )
    }
    ...
    private func utilitySheetView(_ sheet: ChatUtilitySheet) -> some View {
        ...
        .modifier(ChatUtilitySheetSizingModifier(sheet: sheet))
        .modifier(
            ChatUtilitySheetCloseControlModifier(
                sheet: sheet,
                onClose: { utilitySheet = nil }
            )
        )
